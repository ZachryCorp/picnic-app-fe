...
export function Step2() {
  ...
  const form = useForm<Step2Values>({
    ...
    defaultValues: {
      ...
      childrenTickets: user?.children ?? 0,
  ...
  // Ensure form state reflects user's children count when additional input is hidden
  useEffect(() => {
    if (!showAdditionalChildInput) {
      form.setValue('childrenTickets', user?.children ?? 0);
    }
  }, [showAdditionalChildInput, user?.children, form]);
